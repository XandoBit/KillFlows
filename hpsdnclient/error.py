...
def raise_400(response):
    ...
    if "Invalido JSON formato" in data['message']:
...
class HpsdnclientError(Exception):
    """Clase base para el error de la API de la llamaradas"""
    pass
...
class VersionMismatch(HpsdnclientError):
    def __init__(self, dpid, required_version):
        ...
        message = ("Esta característica no se admite en DPID {0}. " +
                   "Se requiere la versión OpenFlow {1}").format(dpid,
                                                              required_version)
...
class OpenflowProtocolError(HpsdnclientError):
    def __init__(self):
        message = ("Algo malo sucedió en la capa de protocolo OpenFlow ." +
                   " Esto podría deberse a que esta característica no está implementada " +
                   "en este dispositivo")
...
class DatatypeError(HpsdnclientError):
    def __init__(self, received, expected):
        ...
        message = "Recivido: {0} Experado: {1}".format(received, expected)
